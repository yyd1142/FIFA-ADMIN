...
class TeamDBAction extends MKODBAction {
  ...
  async getTeam(page = 1, count = 50) {
    ...
    try {
      ...
      if (recordCount > 0) {
        sql = `SELECT a.id , b.open_id , b.gmt_create , b.nation_name , a.winner , c.id lotteryId FROM fifa_competition a LEFT JOIN fifa_team b ON a.id = b.competition_id LEFT JOIN fifa_lottery c ON a.id = c.competition_id WHERE b.open_id IS NOT NULL`;
